...
export const AuthApi = {
    ...
    async sendResetCode(email: string): Promise<any> {
        const response = await ApiClient.post(send_rest_code_end_point, { email: email });
        return response;
    },

    async resetPassword(email: string, resetCode: string, password: string): Promise<any> {
        const response = await ApiClient.post(reset_password_end_point, { email: email, resetCode: resetCode, password: password });
        return response;
    },

    async sendActivationCode(): Promise<any> {
        const response = await ApiClient.post(send_activation_code_end_point, {});
        return response;
    },

    async activateAccount(activationCode: string): Promise<any> {
        const response = await ApiClient.post(activate_account_end_point, { activationCode: activationCode });
        return response;
    },
};
