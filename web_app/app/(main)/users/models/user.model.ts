...
export type User = {
    ...
    /*
     * Organizational read scope.
     *
     * null  = no organizational scope
     * "*"   = all organizations
    ...
    /*
     * Type of organization represented by the scope.
     *
     * "*"   = all organizational unit types
     * null  = no organizational scope
     */
    unitType?: OrgnUnit | "*";
...
export const accessibilityOptions = Object.values(Accessibility).map(
    accessibility => ({
        label: accessibility,
        value: accessibility
    })
...
export const genderOptions = Object.values(Gender).map(
    gender => ({
        label: gender,
        value: gender
    })
...
export const validateUser = (
    ...
    if (
        !user.birthDate ||
        isNaN(new Date(user.birthDate).getTime())
    ) {
        return {
            valid: false,
    ...
    if (user.fin) {
        ...
        if (!finRegex.test(user.fin)) {
            return {
                valid: false,
                message: "FIN must be a 12-digit number."
            };
        }
    }
