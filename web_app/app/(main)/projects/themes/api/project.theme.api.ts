...
import { GetProjectThemeOptions, ProjectTheme, sanitizeProjectTheme } from "../models/project.theme.model";
...
export const ProjectThemeApi = {
    ...
    async getProjectThemes(options: GetProjectThemeOptions): Promise<ProjectTheme[]> {
        ...
        const santized = sanitizeProjectTheme(options);
        if (options.project) query.append("project", santized.project as string);
        const data = await ApiClient.get(`${end_point}?${query.toString()}`);
        return data as ProjectTheme[];
    },

    async create(projectTheme: Partial<ProjectTheme>): Promise<ProjectTheme> {
    ...
    */
    ...
    async delete(projectTheme: Partial<ProjectTheme>): Promise<boolean> {
        if (!projectTheme._id) throw new Error("_id required.");        
        const url = `${end_point}${projectTheme._id}`;
