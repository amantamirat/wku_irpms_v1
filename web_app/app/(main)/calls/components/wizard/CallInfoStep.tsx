...
export const CallInfoStep = ({ data, onUpdate, onNext }: CallInfoStepProps) => {
    ...
    useEffect(() => {
        const loadDependencies = async () => {
            try {
                ...
                console.error('Failed to load Call dependencies:', err);
            }
        };
    ...
    const validateAndNext = () => {
        setSubmitted(true);
        if (data.calendar && data.grant && data.title?.trim()) {
            onNext();
        }
    };
        ...
        <div className="mt-4 p-fluid">
            <div className="grid formgrid">
                ...
                <div className="field col-12 md:col-6">
                    ...
                    <Dropdown
                        ...
                        optionLabel="year"
                        dataKey="_id"
                        onChange={(e) => onUpdate({ ...data, calendar: e.value })}
                        placeholder="Select Year"
                        className={classNames({ 'p-invalid': submitted && !data.calendar })}
                    />
                    {submitted && !data.calendar && <small className="p-error">Operational Year is required.</small>}
                </div>
                ...
                <div className="field col-12 md:col-6">
                    ...
                    <Dropdown
                        ...
                        optionLabel="title"
                        dataKey="_id"
                        onChange={(e) => onUpdate({ ...data, grant: e.value })}
                        placeholder="Select Grant"
                        className={classNames({ 'p-invalid': submitted && !data.grant })}
                    />
                    {submitted && !data.grant && <small className="p-error">Grant selection is required.</small>}
                </div>
            </div>
            ...
            <div className="field">
                ...
                <InputText
                    ...
                    className={classNames({ 'p-invalid': submitted && !data.title?.trim() })}
                    placeholder="e.g., Annual Research Grant Call 2026"
                />
                {submitted && !data.title?.trim() && <small className="p-error">Title is required.</small>}
            </div>
            ...
            <div className="grid formgrid">
                <div className="field col-12 md:col-6">
                    ...
                    <Dropdown
                        ...
                        optionLabel="name"
                        optionValue="_id"
                        onChange={(e) => onUpdate({ ...data, constraint: e.value })}
                ...
                <div className="field col-12 md:col-6">
                    ...
                    <Dropdown
                        ...
                        optionLabel="name"
                        optionValue="_id"
                        onChange={(e) => onUpdate({ ...data, composition: e.value })}
            ...
            <div className="flex justify-content-end mt-5 pt-3 border-top-1 surface-border">
                <Button label="Next: Configure Stages" icon="pi pi-chevron-right" iconPos="right" onClick={validateAndNext} className="px-5" />
            </div>
        </div>
    );
};